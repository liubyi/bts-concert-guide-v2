...
class UberPrices extends React.Component {
  ...
  handleRemoveLocation = idx => () => {
    console.log("handleRemoveLocation");
    this.setState({
  ...
  onButtonClick = async event => {
    ...
    for (let idx = 1; idx < arrLength; idx++) {
      ...
      if (this.state.data_objs[idx].location.dest !== undefined && this.state.data_objs[idx-1].location.dest !== undefined) {
        ...
        .then((response) => {  // what is this response? this?
  ...
  resetState = () => {
      console.log(this.state.data_objs);
     this.setState(this.getInitialState());
     this.handleRemoveLocation(0).bind(this);
  }
